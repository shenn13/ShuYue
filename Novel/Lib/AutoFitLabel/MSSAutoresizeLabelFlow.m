...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.titleLabel.backgroundColor = [UIColor randomColor];
    
    if (indexPath.row < [MSSAutoresizeLabelFlowConfig shareConfig].colors.count) {
        cell.titleLabel.backgroundColor = (UIColor *)[MSSAutoresizeLabelFlowConfig shareConfig].colors[indexPath.row];
    } else {
        cell.titleLabel.backgroundColor = [UIColor randomColor];
    }
...
- (void)layoutFinishWithNumberOfline:(NSInteger)number {
    ...
    //代理
    if ([self.delegate respondsToSelector:@selector(autoLabelHeight:)]) {
        [self.delegate autoLabelHeight:self.collection.height];
    }
}
