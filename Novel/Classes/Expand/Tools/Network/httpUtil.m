...
+ (__kindof NSURLSessionTask *)doRequest:(NSString *)URL
    ...
    NSMutableString *urlStr = [NSMutableString stringWithFormat:@"%@", [URL stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
...
+ (void)getRCIMTTokenWithDict:(NSDictionary *)parameters completed:(void(^)(NSString *token))completed error:(void(^)(NSError *error))errorBlock {
    
    //获取Token的接口
    NSString*urls = @"https://api.cn.ronghub.com/user/getToken.json";
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.requestSerializer  = [AFHTTPRequestSerializer serializer];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/heml",@"application/json",@"text/json",nil];
    manager.requestSerializer = [AFHTTPRequestSerializer serializer];
    
    NSString *appkey = RCIMAppKey;
    NSString *nonce = NSStringFormat(@"%ld", (long)(100000 + (arc4random() % (9999999 - 100000 + 1))));
    NSString *timestamp = [NSString stringWithFormat:@"%d",(int)[[NSDate date] timeIntervalSince1970]];
    NSString *signature = [NSString sha1:[NSString stringWithFormat:@"%@%@%@", RCIMAppSecret, nonce,timestamp]];
    ...
    [manager POST:urls parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        
    } progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
        if ([responseObject[@"code"] integerValue] == 200) {
            
            completed(responseObject[@"token"]);
            
        } else {
            [HUD showMsgWithoutView:@"融云token获取失败"];
        }
...
+ (__kindof NSURLSessionTask *)postForm:(NSString *)URL
                                   form:(void (^)(id <AFMultipartFormData> formData))block
                                success:(HttpRequestSuccess)success
                                failure:(HttpRequestFailed)failure {
    
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:nil constructingBodyWithBlock:block progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"%@",_isOpenLog ? NSStringFormat(@"responseObject = %@",[self jsonToString:responseObject]) : @"Http已关闭日志打印");
