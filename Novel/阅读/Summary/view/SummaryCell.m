...
- (void)setModel:(SummaryModel *)model {
    ...
    if (0 == _row) { //自动选择源
        ...
        _iconV.frame = _iconView.frame;
        _iconV.image = iconImage;
        
        //tilte
        NSMutableAttributedString *titleText = [[NSMutableAttributedString alloc] initWithString:model.source];
        titleText.font = FONT_SIZE(14);
        ...
        _titleLabel.origin = CGPointMake(_iconView.maxX_pro + kSpaceX, (SummaryCellHeight - titleLayout.textBoundingSize.height) * 0.5);
        
        _titleLabel.size = titleLayout.textBoundingSize;
        
        _titleLabel.textLayout = titleLayout;
        
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(kCellX, 0.5, kScreenWidth - kCellX*2, 0.5)];
        lineView.backgroundColor = [UIColor colorWithRed:0.82 green:0.82 blue:0.82 alpha:1.00];
        [self.contentView addSubview:lineView];
        
    } else {
        _chapterLabel.hidden = NO;
