...
/**
 添加书架,直接用小说title's id 作为表名,包含id, summaryId,autoSummaryId,coverURL,title, lastChapter, updated，status, chapter，page，status使用0和1，用来判断有章节更新了是否点击进入阅读界面，作为更新图片是否隐藏的依据
 */
+ (BOOL)addShelfWithModel:(BookShelfModel *)model {
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        ...
        //id设置唯一约束UNIQUE
        res = [db executeUpdate:NSStringFormat(@"create table `%@` (pk integer primary key autoincrement, id text UNIQUE, summaryId text, autoSummaryId text, coverURL text, title text, lastChapter text, updated text, status text, chapter text, page text)",model.id)];
        
        res = [db executeUpdate:NSStringFormat(@"insert into `%@`(id, summaryId, autoSummaryId, coverURL, title, lastChapter, updated, status, chapter, page) values('%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@')",model.id, model.id, model.summaryId, model.autoSummaryId, model.coverURL, model.title, model.lastChapter, model.updated, model.status, model.chapter, model.page)];
        ...
        while ([resultSet next]) {
            ...
            if (![tableName isEqualToString:@"sqlite_sequence"]) {
                ...
                while ([result next]) {
                    ...
                    model.autoSummaryId = [result stringForColumn:@"autoSummaryId"];
